...
df.SalePrice.apply(lambda x: format(x, 'f'))
...
df_train = result = pd.concat([df_train_x,df_train_y], axis=1)
for column in list(df_train.columns.values): 
    hf.missing_median(df_train, column)


df_train_x = df_train.drop('SalePrice', axis=1, inplace=False)
...
y_train_pred = forest.predict(df_train_x)
score = np.sqrt(metrics.mean_squared_error(y_train_pred,df_train_y))
print("Fold score (RMSE): {}".format(math.sqrt(score)))
